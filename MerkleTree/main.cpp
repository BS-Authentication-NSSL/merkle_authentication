...
int main() {
	...
	for(int i = 0; i < files.size(); i++) {
		...
		hashes.at(i) = sha256(getContents(files.at(i)));
	 	...
	 	std::cout << "File " << std::to_string(i + 1) << ": \"" << files.at(i) << "\" has has \"" << hashes.at(i) << "\"" << std::endl;
	}
	// Set the initial ID
	...
	if(tree.root().to_string() == "199eef7383bbba4a1e278cf4aaaf78ed0bb26b3a887a2f6885d976851f5503b2") {
