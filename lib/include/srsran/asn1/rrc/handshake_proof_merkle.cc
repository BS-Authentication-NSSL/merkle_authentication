#ifndef MERKLE_HANDSHAKE_PROOF_CPP
#define MERKLE_HANDSHAKE_PROOF_CPP
...
struct HandshakeProofHashT
{
  ...
  HandshakeProofHashT<SIZE>(const uint8_t* bytes)
  {
  ...
  HandshakeProofHashT<SIZE>(const std::string& s)
  {
    if (s.length() != 2 * SIZE)
      std::cout << "invalid hash string" << std::endl;
  ...
  HandshakeProofHashT<SIZE>(const std::vector<uint8_t>& bytes)
  {
    if (bytes.size() < SIZE)
  ...
  HandshakeProofHashT<SIZE>(const std::vector<uint8_t>& bytes, size_t& position)
  {
  ...
  HandshakeProofHashT<SIZE>(const std::array<uint8_t, SIZE>& bytes)
  {
  ...
  HandshakeProofHashT<SIZE> operator=(const HandshakeProofHashT<SIZE>& other)
  {
  ...
  bool operator==(const HandshakeProofHashT<SIZE>& other) const
  {
  ...
  {
    MERKLECPP_TRACE(MERKLECPP_TOUT << "> HandshakeProofHashT::serialise " << std::endl);
    for (auto& b : bytes)
      buffer.push_back(b);
  }
  ...
  {
    MERKLECPP_TRACE(MERKLECPP_TOUT << "> HandshakeProofHashT::deserialise " << std::endl);
...
template <
  size_t HASH_SIZE,
  void HASH_FUNCTION(
    const HandshakeProofHashT<HASH_SIZE>& l,
    const HandshakeProofHashT<HASH_SIZE>& r,
    HandshakeProofHashT<HASH_SIZE>& out)>
class HandshakeProofPathT
{
public:
  /// @brief HandshakeProofPath direction
  typedef enum
  {
    PATH_LEFT,
    PATH_RIGHT
  ...
  /// @brief HandshakeProofPath element
  typedef struct
  {
    /// @brief The hash of the path element
    HandshakeProofHashT<HASH_SIZE> hash;
    ...
    Direction direction;
  } HandshakeProofElement;

  /// @brief HandshakeProofPath constructor
  /// @param leaf
  /// @param leaf_index
  /// @param elements
  /// @param max_index
  HandshakeProofPathT(
    const HandshakeProofHashT<HASH_SIZE>& leaf,
    size_t leaf_index,
    std::list<HandshakeProofElement>&& elements,
    size_t max_index) :
    _leaf(leaf),
  ...
  /// @brief HandshakeProofPath copy constructor
  /// @param other HandshakeProofPath to copy
  HandshakeProofPathT(const HandshakeProofPathT& other)
  {
    _leaf = other._leaf;
    elements = other.elements;
  }

  /// @brief HandshakeProofPath move constructor
  /// @param other HandshakeProofPath to move
  HandshakeProofPathT(HandshakeProofPathT&& other)
  {
  ...
  HandshakeProofPathT(const std::vector<uint8_t>& bytes)
  {
    deserialise(bytes);
  }
  ...
  /// @param bytes Vector to deserialise from
  /// @param position Position of the first byte in @p bytes
  HandshakeProofPathT(const std::vector<uint8_t>& bytes, size_t& position)
  {
    deserialise(bytes, position);
  }
  ...
  std::shared_ptr<HandshakeProofHashT<HASH_SIZE>> root() const
  {
    std::shared_ptr<HandshakeProofHashT<HASH_SIZE>> result =
      std::make_shared<HandshakeProofHashT<HASH_SIZE>>(_leaf);
    MERKLECPP_TRACE(
      MERKLECPP_TOUT << "> PathT::root " << _leaf.to_string(TRACE_HASH_SIZE)
                      << std::endl);
    for (const HandshakeProofElement& e : elements)
    {
  ...
  /// expected to hash to.
  bool verify(const HandshakeProofHashT<HASH_SIZE>& expected_root) const
  {
  ...
  void deserialise(const std::vector<uint8_t>& bytes, size_t& position)
  {
    ...
    {
      HandshakeProofHashT<HASH_SIZE> hash(bytes, position);
      HandshakeProofPathT::Direction direction =
        bytes[position++] != 0 ? PATH_LEFT : PATH_RIGHT;
      HandshakeProofPathT::HandshakeProofElement e;
      e.hash = hash;
  ...
  size_t serialised_size() const
  {
    return sizeof(_leaf) + elements.size() * sizeof(HandshakeProofElement);
  }
  ...
  const HandshakeProofHashT<HASH_SIZE>& operator[](size_t i) const
  {
    return std::next(begin(), i)->hash;
  }

  /// @brief Iterator for path elements
  typedef typename std::list<HandshakeProofElement>::const_iterator const_iterator;
  ...
  const HandshakeProofHashT<HASH_SIZE>& leaf() const
  {
    return _leaf;
  }

  /// @brief Equality operator for paths
  bool operator==(const HandshakeProofPathT<HASH_SIZE, HASH_FUNCTION>& other) const
  {
  ...
  bool operator!=(const HandshakeProofPathT<HASH_SIZE, HASH_FUNCTION>& other)
  {
...
protected:
  /// @brief The leaf hash
  HandshakeProofHashT<HASH_SIZE> _leaf;
...
template <
  ...
  void HASH_FUNCTION(
    ...
    const HandshakeProofHashT<HASH_SIZE>& r,
    HandshakeProofHashT<HASH_SIZE>& out)>
class HandshakeProofTreeT
{
public: // protected: // Cybersecurity Lab
  /// @brief The structure of tree nodes
  struct HandshakeProofNode
  {
    /// @brief Constructs a new tree node
    /// @param hash The hash of the node
    static HandshakeProofNode* make(const HandshakeProofHashT<HASH_SIZE>& hash)
    {
      auto r = new HandshakeProofNode();
    ...
    static HandshakeProofNode* make(HandshakeProofNode* left, HandshakeProofNode* right)
    {
      assert(left && right);
      auto r = new HandshakeProofNode();
      r->left = left;
    ...
    /// @param from HandshakeProofNode to copy
    ...
    static HandshakeProofNode* copy_node(
      const HandshakeProofNode* from,
      std::vector<HandshakeProofNode*>* leaf_nodes = nullptr,
      ...
      size_t indent = 0)
    {
      ...
      HandshakeProofNode* r = make(from->hash);
      r->size = from->size;
    ...
    HandshakeProofNode* left;

    /// @brief The right child of the node
    HandshakeProofNode* right;
...
public:
  ...
  typedef HandshakeProofHashT<HASH_SIZE> Hash;

  /// @brief The type of paths in the tree
  typedef HandshakeProofPathT<HASH_SIZE, HASH_FUNCTION> HandshakeProofPath;

  /// @brief The type of the tree
  typedef HandshakeProofTreeT<HASH_SIZE, HASH_FUNCTION> Tree;

  /// @brief Constructs an empty tree
  HandshakeProofTreeT() {}

  /// @brief Copies a tree
  HandshakeProofTreeT(const HandshakeProofTreeT& other)
  {
    *this = other;
  }
  ...
  HandshakeProofTreeT(HandshakeProofTreeT&& other) :
  ...
  HandshakeProofTreeT(const std::vector<uint8_t>& bytes)
  {
    deserialise(bytes);
  }
  ...
  HandshakeProofTreeT(const std::vector<uint8_t>& bytes, size_t& position)
  {
    deserialise(bytes, position);
  }
  ...
  HandshakeProofTreeT(const Hash& root)
  {
    insert(root);
  }
  ...
  void insert(const Hash& hash)
  {
    MERKLECPP_TRACE(MERKLECPP_TOUT << "> insert "
                                    ...
                                    << std::endl;);
    uninserted_leaf_nodes.push_back(HandshakeProofNode::make(hash));
  ...
  void flush_to(size_t index)
  {
    ...
    walk_to(index, false, [this](HandshakeProofNode*& n, bool go_right) {
      if (go_right && n->left)
      {
  ...
  void retract_to(size_t index)
  {
    ...
    if (index < min_index())
      std::cout << "leaf index out of bounds" << std::endl;
    ...
    HandshakeProofNode* new_leaf_node =
      walk_to(index, true, [this](HandshakeProofNode*& n, bool go_right) {
        ...
        if (go_left && n->right)
        {
          ...
          HandshakeProofNode* old_left = n->left;
          delete (n->right);
          n->right = nullptr;
  ...
  Tree& operator=(const Tree& other)
  {
    ...
    _root = HandshakeProofNode::copy_node(
      other._root,
      &leaf_nodes,
      &to_skip,
      other.min_index(),
    ...
    for (auto n : other.uninserted_leaf_nodes)
      uninserted_leaf_nodes.push_back(HandshakeProofNode::copy_node(n));
    num_flushed = other.num_flushed;
  ...
  std::shared_ptr<Hash> past_root(size_t index)
  {
    ...
    for (auto e : *p)
      if (e.direction == HandshakeProofPath::Direction::PATH_LEFT)
        HASH_FUNCTION(e.hash, *result, *result);
  ...
  inline HandshakeProofNode* walk_to(
    size_t index, bool update, const std::function<bool(HandshakeProofNode*&, bool)>&& f)
  {
    if (index < min_index() || max_index() < index)
      std::cout << "invalid leaf index" << std::endl;

    compute_root();

    assert(index < _root->size);

    HandshakeProofNode* cur = _root;
    size_t it = 0;
    if (_root->height > 1)
  ...
  /// @return The path
  std::shared_ptr<HandshakeProofPath> path(size_t index)
  {
    MERKLECPP_TRACE(MERKLECPP_TOUT << "> path from " << index << std::endl;);
    statistics.num_paths++;
    std::list<typename HandshakeProofPath::HandshakeProofElement> elements;

    walk_to(index, false, [&elements](HandshakeProofNode* n, bool go_right) {
      typename HandshakeProofPath::HandshakeProofElement e;
      e.hash = go_right ? n->left->hash : n->right->hash;
      e.direction = go_right ? HandshakeProofPath::PATH_LEFT : HandshakeProofPath::PATH_RIGHT;
      elements.push_front(std::move(e));
      return true;
    });

    return std::make_shared<HandshakeProofPath>(
  ...
  std::shared_ptr<HandshakeProofPath> past_path(size_t index, size_t as_of)
  {
    ...
    if (
      ...
      std::cout << "invalid leaf indices" << std::endl;
    ...
    std::list<typename HandshakeProofPath::HandshakeProofElement> root_to_fork, fork_to_index,
      fork_to_as_of;
    HandshakeProofNode* fork_node = nullptr;

    HandshakeProofNode *cur_i = _root, *cur_a = _root;
      ...
      if (!fork_node)
      {
        ...
        if (cur_i->height == height)
        {
          if (go_right_i)
          {
            ...
            e.direction = go_right_i ? HandshakeProofPath::PATH_LEFT : HandshakeProofPath::PATH_RIGHT;
      ...
      else
      {
        ...
        if (cur_a->height == height)
        {
          ...
          if (go_right_a)
          {
            ...
            e.direction = go_right_a ? HandshakeProofPath::PATH_LEFT : HandshakeProofPath::PATH_RIGHT;
    ...
    std::list<typename HandshakeProofPath::HandshakeProofElement> path;
    ...
    if (fork_node)
    {
      ...
      typename HandshakeProofPath::HandshakeProofElement e;
      e.hash = as_of_hash;
      e.direction = HandshakeProofPath::PATH_RIGHT;
  ...
  void serialise(std::vector<uint8_t>& bytes)
  {
    ...
    if (!empty())
    {
      ...
      std::vector<HandshakeProofNode*> extras;
      walk_to(min_index(), false, [&extras](HandshakeProofNode*& n, bool go_right) {
        if (go_right)
  ...
  void serialise(size_t from, size_t to, std::vector<uint8_t>& bytes)
  {
    ...
    if (!empty())
    {
      ...
      std::vector<HandshakeProofNode*> extras;
      walk_to(from, false, [&extras](HandshakeProofNode*& n, bool go_right) {
        if (go_right)
          extras.push_back(n->left);
        return true;
      });
  ...
  void deserialise(const std::vector<uint8_t>& bytes, size_t& position)
  {
    ...
    {
      HandshakeProofNode* n = HandshakeProofNode::make(bytes.data() + position);
      position += HASH_SIZE;
      leaf_nodes.push_back(n);
    }

    std::vector<HandshakeProofNode*> level = leaf_nodes, next_level;
    ...
    while (it != 0 || level.size() > 1)
    {
      ...
      if (it & 0x01)
      {
        ...
        MERKLECPP_TRACE(MERKLECPP_TOUT << "+";);
        auto n = HandshakeProofNode::make(h);
        n->height = level_no + 1;
        n->size = (1 << n->height) - 1;
        assert(n->invariant());
      ...
      for (size_t i = 0; i < level.size(); i += 2)
      {
        if (i + 1 >= level.size())
          ...
          next_level.push_back(HandshakeProofNode::make(level[i], level[i + 1]));
      }
  ...
  size_t serialised_size()
  {
    ...
    if (!empty())
    {
      walk_to(min_index(), false, [&num_extras](HandshakeProofNode*&, bool go_right) {
        if (go_right)
          num_extras++;
        return true;
      });
  ...
  {
    size_t num_extras = 0;
    walk_to(from, false, [&num_extras](HandshakeProofNode*&, bool go_right) {
      if (go_right)
        num_extras++;
      return true;
    });
  ...
  std::string to_string(size_t num_bytes = HASH_SIZE) const
  {
    ...
    std::stringstream stream;
    std::vector<HandshakeProofNode*> level, next_level;
...
protected:
  ...
  std::vector<HandshakeProofNode*> leaf_nodes;

  /// @brief Vector of leaf nodes to be inserted in the tree
  /// @note These nodes are conceptually inserted, but no HandshakeProofNode objects have
  /// been inserted for them yet.
  std::vector<HandshakeProofNode*> uninserted_leaf_nodes;

  /// @brief Number of flushed nodes
  size_t num_flushed = 0;

  /// @brief Current root node of the tree
  HandshakeProofNode* _root = nullptr;
...
private:
  ...
  mutable std::vector<HandshakeProofNode*> hashing_stack;

  /// @brief The walk stack
  /// @note To avoid actual recursion, this holds the stack/continuation for
  /// walking down the tree from the root to a leaf.
  mutable std::vector<HandshakeProofNode*> walk_stack;

public: // protected: // Cybersecurity Lab
  /// @brief Finds the leaf node corresponding to @p index
  /// @param index The leaf node index
  const HandshakeProofNode* leaf_node(size_t index) const
  {
  ...
  /// hashes, if required.
  void hash(HandshakeProofNode* n, size_t indent = 2) const
  {
  ...
  void compute_root()
  {
    insert_leaves(true);
    if (num_leaves() == 0)
      std::cout << "empty tree does not have a root" << std::endl;
    assert(_root);
    assert(_root->invariant());
    if (_root->dirty)
    {
  ...
  /// @note This adds one new HandshakeProofNode to the insertion stack/continuation for
  /// efficient processing by process_insertion_stack() later.
  void continue_insertion_stack(HandshakeProofNode* n, HandshakeProofNode* new_leaf)
  {
    while (true)
    {
      MERKLECPP_TRACE(MERKLECPP_TOUT << "  @ "
      ...
      if (n->is_full())
      {
        HandshakeProofNode* result = HandshakeProofNode::make(n, new_leaf);
  ...
  /// should be processed
  HandshakeProofNode* process_insertion_stack(bool complete = true)
  {
    MERKLECPP_TRACE({
      std::string nodes;
    ...
    HandshakeProofNode* result = insertion_stack.back().n;
  ...
  void insert_leaf(HandshakeProofNode*& root, HandshakeProofNode* n)
  {
...
static inline void sha256_compress(const HandshakeProofHashT<32> &l, const HandshakeProofHashT<32> &r, HandshakeProofHashT<32> &out) {
...
static inline void sha256_compress_openssl(
  const HandshakeProofHashT<32>& l, const HandshakeProofHashT<32>& r, HandshakeProofHashT<32>& out)
{
...
static inline void sha256_openssl(
  const HandshakeProofHashT<32>& l,
  const HandshakeProofHashT<32>& r,
  HandshakeProofHashT<32>& out)
{
  uint8_t block[32 * 2];
  memcpy(&block[0], l.bytes, 32);
  memcpy(&block[32], r.bytes, 32);
...
typedef HandshakeProofHashT<32> HandshakeProofHash;

/// @brief Type of paths in the default tree type
typedef HandshakeProofPathT<32, sha256_compress> HandshakeProofPath;

/// @brief Default tree with default hash size and function
typedef HandshakeProofTreeT<32, sha256_compress> HandshakeProofTree;

#endif // MERKLE_HANDSHAKE_PROOF_CPP